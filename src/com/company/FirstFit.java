...
public class FirstFit {
    ...
    int[] allocatprocess;
    String[] allocatpart;
    public FirstFit(int numpro,int numpart) {
        numofpart =numpart;
        numofprocess = numpro;
    ...
    public void firstfit(Process process [], Partition part[])
    {
        int cont = numofpart;
        ...
        for(int i = 0 ; i < numofprocess;i++)
        {
            for(int j = 0 ; j < numofpart;j++)
            {
                ...
                {
                    allocatpart[i]=part[j].name;
                    allocatprocess[i]=j;
                    part[j].name= "partition"+cont;
                    cont++;
                    allocatprocess[i]=j;
                    part[j].size-=process[i].size;
                    break;
                }
            }
        }
            ...
            for(int j = 0 ; j < numofprocess ; j++)
            {
                if(allocatprocess[j] == -1 )
                {
                   if(compactedPartion>process[j].size){
                        allocatpart[j]="Partition";

                        System.out.println(allocatpart[j] +cont+ " ("+process[j].size + "KB) ==> "+process[j].name);
